...
extension Diagnostic {
    ...
    static func openAPIVersionError(versionString: String, location: Location) -> Diagnostic {
        return error(
            message:
                "Unsupported document version: \(versionString). Please provide a document with OpenAPI versions in the 3.1.0 set.",
            location: location
        )
    }
    ...
    static func openAPIMissingVersionError(location: Location) -> Diagnostic {
        return error(
            message:
                "No openapi key found, please provide a valid OpenAPI document with OpenAPI versions in the 3.1.0 set.",
            location: location
        )
    }
}
